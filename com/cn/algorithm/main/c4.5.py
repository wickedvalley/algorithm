...
def splitDataSet(dataSet, axis, value): #axis是dataSet数据集下要进行特征划分的列号例如outlook是0列，value是该列下某个特征值，0列中的sunny
    ...
    for featVec in dataSet: #遍历数据集，并抽取按axis的当前value特征进划分的数据集(不包括axis列的值)
        if featVec[axis] == value: #
            ...
            reducedFeatVec.extend(featVec[axis+1:])#extend会扩充
...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):        #遍历每个特征iterate over all the features
        ...
        for value in uniqueVals: #计算每种划分方式的信息熵
            subDataSet = splitDataSet(dataSet, i, value)#如i=0，value=sunny
...
def createTree(dataSet,labels):
    ...
    featValues = [example[bestFeat] for example in dataSet]#当前特征列的所有值，如outlook下的['sunny', 'sunny', 'overcast'...]
    print featValues
    uniqueVals = set(featValues)#['sunny','overcast','rainy']
    for value in uniqueVals:#对于每个分支，递归的创建树
        subLabels = labels[:]       #复制剩余的标签 ['temperature','huminidy','windy']
...
if __name__ == '__main__':
    filePath=os.getcwd()+"\\play.tennies.txt"
    fr = open(filePath)
